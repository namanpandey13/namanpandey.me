  ...
  return (
    <div className="grid gap-7 md:grid-cols-2">
      {appearances.map((appearance, index) => (
        <AppearanceCard key={appearance.href} appearance={appearance} index={index} />
      ))}
    </div>
  );
}
...
function AppearanceCard({
  ...
  const imageSrc = appearance.image.startsWith(site.url)
    ? appearance.image.replace(site.url, "")
    : appearance.image;
